...
submitImageForm() {
      ...
      let imageTitle = this.getImageTitle(id);
        if(this.imageUploadForm.valid) {
          ...
          this.userRepository.saveProfileImage(formData, id, imageTitle);
...
this.userRepository.setActiveProfilePhoto( this.tokenService.getDecodedToken().id, imageName);
}

deleteProfileImage(imageName: string) {
this.userRepository.deleteProfileImage( this.tokenService.getDecodedToken().id, imageName);
